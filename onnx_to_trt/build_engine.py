...
from onnx_to_trt.image_batcher import ImageBatcher
...
class EngineBuilder:
    ...
    def set_mixed_precision_myself(self, start_idx, end_idx):
        """
        Experimental precision mode.
        Enable mixed-precision mode. When set, the layers defined here will be forced to FP16 to maximize
        INT8 inference accuracy, while having minimal impact on latency.
        根据层的索引来选择量化精度，适合于层数多且需要成片设置精度的模型
        """
        self.config.set_flag(trt.BuilderFlag.PREFER_PRECISION_CONSTRAINTS)
        self.config.set_flag(trt.BuilderFlag.DIRECT_IO)
        self.config.set_flag(trt.BuilderFlag.REJECT_EMPTY_ALGORITHMS)
        ...
        for i in range(start_idx, min(end_idx, self.network.num_layers)):
            layer = self.network.get_layer(i)
            
            # # 跳过不支持的层类型
            # if layer.type in unsupported_types:
            #     continue
        ...
        # if layer.type in [trt.LayerType.CONVOLUTION]:
            ...
            log.info(f"Layer {i} ({layer.name}) set to FP32 (Skipped Quantization)")
...
def main(args):
    ...
    if args.precision == "mixed":
        print("进入混合编译模式")
        builder.set_mixed_precision_myself(318, 476)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-o", "--onnx", default="./onnx_model/yolo11n_cls_train_12_3x640_wnms_bias.onnx", help="The input ONNX model file to load"
    )
    parser.add_argument(
        "-e", "--engine", default="./trt_model/yolo11n_cls_train_12_3x640_wnms_1041_int8_mixed.trt", help="The output path for the TRT engine"
    )
    parser.add_argument(
        "-b",
        "--batch_size",
    ...
    parser.add_argument(
        "--calib_input", default="/media/hc/DataDisk/data/Rally/detect/yolo11_cls_20260110/images/train/", help="The directory holding images to use for calibration"
    )
    parser.add_argument(
        "--calib_cache",
        default="./int8_cache_file/yolo11n_cls_train_12_3x640_wnms_1041_int8.cache",
